	...
	if(isset ($_POST["radioPesquisar"])){
		...
		if(isset($_POST["txtProduto"])){
			if($radioValue == 1)
			{
				...
				$tabela_grid_dinamica = '<form action="recebe.php" method="POST" id="formRadio"> <table cellpadding="10px" class="table table-condensed table-hover">';
				...
				$tabela_grid_dinamica .= '</tr></table></form>';
			}elseif($radioValue == 2){
				...
				$sql = mysqli_query($conexao,"SELECT nome,cnpj,telefone,rua,numero,bairro,cidade from fornecedor,endereco where nome like'%$key%' and fornecedor.id_endereco = endereco.id_endereco");
